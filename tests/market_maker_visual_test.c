...
static void print_book(order_book_t* book, const char* msg, int num_noise, int num_mm)
{
  ...
  printf("  ╔════════════════════════════════════════════════════════╗\n");
  printf("  ║         📊 LOB SIM: NOISE + MARKET MAKERS 📊          ║\n");
  printf("  ╚════════════════════════════════════════════════════════╝\n\n" COLOR_RESET);
  ...
  // Stats
  size_t total_orders = book->orders.count;
  printf(COLOR_MAGENTA "       🎲 Noise Traders: %d    🏦 Market Makers: %d\n" COLOR_RESET,
         num_noise, num_mm);
...
int main(void)
{
  ...
  for (int t = 0; t < total_ticks; t += display_every)
  {
    ...
    print_book(&book, msg, num_noise, num_mm);
    usleep(80000);
  }
