...
static void expect_not_found(const price_tree_t* t, price_t p) { assert(pt_find(t, p) == NULL); }
...
static void test_remove_leaf_one_child_two_children(void)
{
  ...
  // Remove a node with one child (11 after removing 13? often one-child/leaf depending on
  // rotations)
  ...
  for (int i = 0; i < N; i++)
  {
    if (keys[i] == 10 || keys[i] == 11 || keys[i] == 13)
      continue;
