...
static void print_trade(trade_t* trade)
{
  printf(COLOR_YELLOW
         ...
         (long long)trade->qty, (long long)trade->price, (unsigned long long)trade->buy_id,
         (unsigned long long)trade->sell_id);
}
...
int main(void)
{
  ...
  snprintf(msg, sizeof(msg), "BUY order: %lld @ %lld (adding to existing level)",
           (long long)b4->qty, (long long)b4->price);
  print_book(&book, msg);
  SLEEP_MS(ANIMATION_DELAY_MS);
